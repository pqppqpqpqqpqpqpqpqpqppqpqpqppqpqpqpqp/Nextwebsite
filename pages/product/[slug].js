...
const Post = ({ buyNow, addToCart, product, variants }) => {
  ...
  const checkService = async () => {
    ...
    if (pinJson.includes(parseInt(pin))) {
      setService(true)
      toast.success('Your pincode is active', {
      ...
      setService(false)
      toast.error('Sorry! Your pincode is not active', {
        position: "bottom-center",
        autoClose: 1000,
        hideProgressBar: false,
        closeOnClick: true,
          ...
          <div className="lg:w-1/2 w-full  lg:pl-10 lg:py-6 mt-6 lg:mt-0">
            ...
            <div className="flex mt-6 items-center pb-5 border-b-2 border-gray-100 mb-5">
              <div className="flex">
                ...
                {Object.keys(variants).includes('green') && Object.keys(variants['green']).includes(size) && <button onClick={() => { refreshVariant(size, 'green') }} className={`border-2  ml-1 bg-green-500   rounded-full w-6 h-6 focus:outline-none ${color === 'green' ? 'border-black' : 'border-gray-700'} `}></button>}
                {Object.keys(variants).includes('blue') && Object.keys(variants['blue']).includes(size) && <button onClick={() => { refreshVariant(size, 'blue') }} className={`border-2  ml-1 bg-blue-500   rounded-full w-6 h-6 focus:outline-none ${color === 'blue' ? 'border-black' : 'border-gray-700'} `}></button>}
                {Object.keys(variants).includes('purple') && Object.keys(variants['purple']).includes(size) && <button onClick={() => { refreshVariant(size, 'purple') }} className={`border-2  ml-1 bg-purple-500   rounded-full w-6 h-6 focus:outline-none ${color === 'purple' ? 'border-black' : 'border-gray-700'} `}></button>}
           ...
           <div className="flex ml-6 items-center">
            ...
            <div className="flex ">
              ...
              <button onClick={() => { buyNow(slug, 1, product.price, product.title, size, color) }} className="flex ml-8 text-white bg-pink-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded capitalize">buy now</button>
              <button onClick={() => { addToCart(slug, 1, product.price, product.title, size, color) }} className="flex ml-4 text-white bg-pink-500 border-0 py-2 px-2 md:px-6 focus:outline-none hover:bg-pink-600 rounded capitalize">add to cart</button>
            ...
            <div className='pin mt-6 flex space-x-2 text-sm'>
              <input onChange={onChangePin} className='px-2 border-2 border-pink-700 rounded' placeholder='Enter your pincode' type='text' />
              <button onClick={checkService} className='capitalize text-white bg-pink-500 border-0 py-2 px-6 focus:outline-none hover:bg-pink-600 rounded '>check</button>
            </div>
